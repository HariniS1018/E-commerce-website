...
$sql_cursor=mysqli_query($conn,"select cart.quantity,cart.cartid,cart.pid,product.name,product.price,product.details,product.discount,product.impath from cart join product on cart.pid=product.pid where cart.pid in (select cart.pid from cart where cart.cartid= $cartid);");
...
while($row=mysqli_fetch_assoc($sql_cursor)){
    ...
    echo "<form action='editted_cart.php' method='post'>
            <div class='card'>
                <div class='card-body'>
                    <input class='w-25 border-0' readonly type='hidden'name='cartid' value='$cartid'>";
